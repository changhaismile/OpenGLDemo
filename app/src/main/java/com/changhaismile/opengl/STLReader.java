...
public class STLReader {
    ...
    public Model parserBinStl(InputStream in) throws IOException {
        ...
        int facetCount = Utils.byte4ToInt(bytes, 0);
    ...
    private void parseModel(Model model, byte[] facetBytes) {
        ...
        try {
            for (int i = 0; i < facetCount; i++) {
                ...
                for (int j = 0; j < 4; j++) {
                    float x = Utils.byte4ToFloat(facetBytes, stlOffset);
                    float y = Utils.byte4ToFloat(facetBytes, stlOffset + 4);
                    float z = Utils.byte4ToFloat(facetBytes, stlOffset + 8);
                    stlOffset += 12;
                ...
                short r = Utils.byte2ToShort(facetBytes, stlOffset);
